...
async def _generate(request_dict: dict, raw_request: Request) -> Response:
    ...
    results_generator = engine.generate(prompt=inputs,
    ...
    async def stream_results() -> AsyncGenerator[bytes, None]:
        async for request_output in results_generator:
            ...
            output_tokens = [
                list(output.token_ids) for output in request_output.outputs
            ]
            logprobs = [[{
                k: asdict(v)
                for k, v in logprobs.items()
            } for logprobs in output.logprobs]
                        if output.logprobs is not None else None
    ...
    logprobs = [[{
        k: asdict(v)
        for k, v in logprobs.items()
    } for logprobs in output.logprobs] if output.logprobs is not None else None
